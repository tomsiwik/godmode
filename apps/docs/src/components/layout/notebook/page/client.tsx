  ...
  return (
    ...
    >
      <Collapsible
        ...
        data-toc-popover=""
        className={cn(
          'sticky top-(--fd-docs-row-2) z-10 [grid-area:toc-popover] h-(--fd-toc-popover-height) xl:hidden max-xl:layout:[--fd-toc-popover-height:--spacing(14)]',
          className,
        )}
        {...rest}
      >
        <header
          ref={ref}
          className={cn(
            'border-b py-2 backdrop-blur-sm transition-colors',
...
export function PageTOCPopoverContent(props: ComponentProps<'div'>) {
  return (
    <CollapsibleContent
      ...
      {...props}
      className={cn('flex flex-col px-4 py-3 max-h-[50vh] md:px-6', props.className)}
    >
      {props.children}
    </CollapsibleContent>
  );
